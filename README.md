...
**Note**: The code generated by Replit after generating the app and iteratively refining it through my prompts is also included on the main branch. 
...
1) The link between XamCent and the VSS via its RestAPI backend seems to not be as stable, causing XamCent to often return with totally irrelevant questions. A suitable measure to mitigate this may involve adding an API endpoint such as '''/generate_exam''' amidst other functionalities offered.
2) In the interest of time, post-training/SFT Cosmos-Reason-2-2B or the Llama-3.1-70B instruct models could not be undertaken. Using QLoRA FT on open-source benchmarks such as COIN (Comprehensive Instructional Videos), HowTo100M, or EduQG, pre-processed to align with the Llava dataset format, may help specialize these models from the VSS workflow to capture intricacies within key concepts discussed in a tutorial, and generate trick-questions based on that, or learn how to create plausible distractors/closely-matching options in the trivia.
